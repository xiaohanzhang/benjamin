...
export function generateBar(id: number, cell: number): Bar {
  ...
  const x = Math.floor(Math.random() * (GRID_W - 1)) // 0..GRID_W-2 (needs 2 cols)
...
export function spawn(g: Game, cell: number) {
  ...
  const bar = generateBar(g.nid++, cell)
...
export function tick(g: Game, dt: number, t: number, cell: number): TickResult {
  ...
  // Freeze falling while shot animation plays
  if (!shooting) {
    ...
    // Spawn
    const maxB = Math.min(5, 1 + Math.floor(g.score / 3))
    const interval = Math.max(MIN_SPAWN_MS, BASE_SPAWN_MS - g.score * SPAWN_DEC)
    if (g.bars.length < maxB && t - g.lastSpawn >= interval) { spawn(g, cell); g.lastSpawn = t }
    if (g.bars.length === 0) { spawn(g, cell); g.lastSpawn = t }
  }
