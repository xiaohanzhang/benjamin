...
import { GRID_H, COLORS, CANNON_FILES, LEVEL_UP_MS } from './constants'
import { findTarget, barHeight, barWidth, cannonTier } from './logic'
import {
  rrect, drawSky, drawGround, drawGridLines, drawColumnHighlight,
  drawFx, drawLevelUpOverlay,
...
export function renderFrame(
  ...
  // Grid lines
  drawGridLines(ctx, gridW, groundY, cell)

  // Ground
  drawGround(ctx, cw, groundY, cell)

  // Column highlight under cannon
  drawColumnHighlight(ctx, g.cannonX, groundY, cell)
  ...
  // FX text
  drawFx(ctx, g.fx, t, cell)

  // Level-up overlay
  drawLevelUpOverlay(ctx, g, t, cell, cw, ch, LEVEL_UP_MS)
