    ...
    <div className="space-y-6">
      ...
      <div className="rounded-3xl bg-white/80 shadow-xl p-6">
        ...
        <div className="flex items-center gap-4 mb-4 text-xs text-gray-500">
          ...
          <span className="flex items-center gap-1">
            <span className="inline-block w-3 h-3 rounded-full" style={{ background: COLORS.phonics }} /> Phonics
          </span>
        </div>
...
function ActivityChart({ data }: { data: DailyActivity[] }) {
  ...
  const phonicsLine = mkLine(data.map(d => d.phonics))
  ...
  return (
    <div className="overflow-x-auto">
      <svg viewBox={`0 0 ${W} ${H + 30}`} className="w-full" style={{ minWidth: '300px' }}>
        ...
        <path d={phonicsLine} fill="none" stroke={COLORS.phonics} strokeWidth="1.5" strokeLinejoin="round" strokeDasharray="4 2" />
        ...
        {data.map((d, i) => <Dot key={`tp${i}`} cx={x(i)} cy={y(d.phonics)} r="2" fill={COLORS.phonics} />)}
        ...
        {data.map((d, i) => <Tip key={`hp${i}`} cx={x(i)} cy={y(d.phonics)} fill={COLORS.phonics} label={`Phonics: ${d.phonics}`} />)}
